...
const User = {
  login: (user: IAdminUserForm): Promise<IAdminUser> =>
    requests.post(`/user/login`, user),
};

const Server = {
  getAll: (): Promise<IGameServer[]> => requests.get("/server/all"),
  getServer: (id: string) : Promise<IGameServer|null> => requests.get(`/server/${id}`),
};
